...
module.exports = {
    ...
    run: async (client, message, args) => {
      if (!message.member.permissions.has("KICK_MEMBERS")) {
        return message.reply({embeds: [new MessageEmbed().setColor(client.color).setDescription(`<:icons_reddet:1235288367491711016> | You must have \`Kick Members\` permissions to use this command.`)]});
      }
      ...
      if(user.id === client.user.id) return message.reply({embeds: [new MessageEmbed().setColor(client.color).setDescription(`<:icons_reddet:1235288367491711016> | You can't kick me.`)]})
      
      if(user.id === message.guild.ownerId) return message.reply({embeds: [new MessageEmbed().setColor(client.color).setDescription(`<:icons_reddet:1235288367491711016> | I can't kick the owner of this server.`)]})
      ...
      if(!user.kickable){
        ...
        .setDescription(`<:icons_reddet:1235288367491711016> | I can't kick this user.`)
        .setColor(client.color)
        return message.reply({embeds: [embed]})
    }
    user.kick({reason: rea})
    const done = new MessageEmbed()
    .setDescription(`<:IconTick:1229080348277604456> | Successfully kicked **${user.user.tag}** from the server.`)
    .setColor(client.color)
