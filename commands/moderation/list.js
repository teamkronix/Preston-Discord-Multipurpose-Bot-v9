...
module.exports = {
 ...
 member.permissions.has('ADMINISTRATOR')
    ...
    if (require === "bot" || require == 'bots') {
      ...
      if(!bot) return message.reply({embeds: [new MessageEmbed().setColor(client.color).setDescription(`<:icons_reddet:1235288367491711016> | I guess there is no bots in this server.`)]});
      const embed = new MessageEmbed()
    ...
    if (require === "inrole" || require == 'role') {
      ...
      if(!role) return message.reply({embeds: [new MessageEmbed().setColor(client.color).setDescription(`<:icons_reddet:1235288367491711016> | No roles found.`)]});
      const embed = new MessageEmbed()
