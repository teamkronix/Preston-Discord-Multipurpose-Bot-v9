...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if(!message.mentions.members.first()){
      return message.reply({embeds: [new MessageEmbed().setColor(client.color).setDescription(`You must mention someone whom you want to mute in your vc.`)]})
    }
    ...
    try{
      ...
      message.reply({embeds: [new MessageEmbed().setColor(client.color).setDescription(`<:IconTick:1229080348277604456> | Successfully Muted <@${member.user.id}> From Voice!`)]})
    }catch(err){
      return message.reply({embeds: [new MessageEmbed().setColor(client.color).setDescription(`I was unable to voice mute <@${member.user.id}>.`)]})
    }
  }
}