...
module.exports = class Util {
  ...
  async buildGreeting(member, type, config) {
    ...
    if (!config.content && !config.embed.description && !config.embed.footer) {
      return { content: `<@${member.user.id}>`, embeds: [new MessageEmbed().setColor(this.client.color).setDescription(`Hey ${member.displayName}, Welcome to the server <a:welcome:1119640001819463740>`)] };
    }
  ...
  hasHigher(member){
    ...
    async selectMenuHandle(interaction) {
      try {
          ...
          if (funny === 'first') {
            _commands = this.client.commands.filter((x) => x.category && x.category === "security").map((x) => `\`${x.name}\``);
            embed.addField(`**Antinuke \`[${_commands.length}]\`**`, _commands.sort().join(", "))
            interaction.update({
              embeds: [embed],
              ephemeral: true
            }).catch((_) => { })
          ...
          if (funny === 'second') {
            _commands = this.client.commands.filter((x) => x.category && x.category === "mod").map((x) => `\`${x.name}\``);
            embed.addField(`**Moderation \`[${_commands.length}]\`**`, _commands.sort().join(", "))
            interaction.update({
              embeds: [embed],
              ephemeral: true
            }).catch((_) => { })
          ...
          if (funny === 'third') {
            _commands = this.client.commands.filter((x) => x.category && x.category === "info").map((x) => `\`${x.name}\``);
            embed.addField(`**Utility \`[${_commands.length}]\`**`, _commands.sort().join(", "))
            interaction.update({
              embeds: [embed],
              ephemeral: true
            }).catch((_) => { })
          ...
          if (funny === 'fourth') {
            _commands = this.client.commands.filter((x) => x.category && x.category === "fun").map((x) => `\`${x.name}\``);
            embed.addField(`**Fun \`[${_commands.length}]\`**`, _commands.sort().join(", "))
            interaction.update({
              embeds: [embed],
              ephemeral: true
            }).catch((_) => { })
          ...
          if (funny === 'fifth') {
            _commands = this.client.commands.filter((x) => x.category && x.category === "giveaway").map((x) => `\`${x.name}\``);
            embed.addField(`**Giveaway \`[${_commands.length}]\`**`, _commands.sort().join(", "))
            interaction.update({
              embeds: [embed],
              ephemeral: true
            }).catch((_) => { })
          ...
          if (funny === 'sixth') {
            _commands = this.client.commands.filter((x) => x.category && x.category === "welcomer").map((x) => `\`${x.name}\``);
            embed.addField(`**Welcomer \`[${_commands.length}]\`**`, _commands.sort().join(", "))
            interaction.update({
              embeds: [embed],
              ephemeral: true
            }).catch((_) => { })
